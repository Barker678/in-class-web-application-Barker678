{extend file="layouts/main.tpl"}
{block name="body"}
<div class="hero" style="background-image: url('./images/banner_1.jpg');">
            <div class="container">
                <h1>Student Recipes</h1>
            </div>
        </div>

        <div class="container recipe-grid mt-5">
            <div class="row">
                <div class="col-md-6 col-lg-4">
                    <div class="card mb-3">
                        <div class="recipe-card-image" style="background-image: url('./user_images/1.jpg');"></div>
                        <div class="card-body">
                            <h5 class="card-title">Breakfast</h5>
                            <p class="card-text">This lovely salad is quick to make and tastes amazing!</p>
                            <a href="recipes.html" class="btn btn-primary">View breakfast recipes</a>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4">
                    <div class="card mb-3">
                        <div class="recipe-card-image" style="background-image: url('./user_images/2.jpg');"></div>
                        <div class="card-body">
                            <h5 class="card-title">Lunch</h5>
                            <p class="card-text">This lovely salad is quick to make and tastes amazing!</p>
                            <a href="recipes.html" class="btn btn-primary">View lunch recipes</a>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4">
                    <div class="card mb-3">
                        <div class="recipe-card-image" style="background-image: url('./user_images/3.jpg');"></div>
                        <div class="card-body">
                            <h5 class="card-title">Dinner</h5>
                            <p class="card-text">This lovely salad is quick to make and tastes amazing!</p>
                            <a href="recipes.html" class="btn btn-primary">View dinner recipes</a>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4">
                    <div class="card mb-3">
                        <div class="recipe-card-image" style="background-image: url('./user_images/4.jpg');"></div>
                        <div class="card-body">
                            <h5 class="card-title">Light Bites</h5>
                            <p class="card-text">This lovely salad is quick to make and tastes amazing!</p>
                            <a href="recipes.html" class="btn btn-primary">View light bite recipes</a>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4">
                    <div class="card mb-3">
                        <div class="recipe-card-image" style="background-image: url('./user_images/1.jpg');"></div>
                        <div class="card-body">
                            <h5 class="card-title">Snacks</h5>
                            <p class="card-text">This lovely salad is quick to make and tastes amazing!</p>
                            <a href="recipes.html" class="btn btn-primary">View snack recipes</a>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4">
                    <div class="card mb-3">
                        <div class="recipe-card-image" style="background-image: url('./user_images/2.jpg');"></div>
                        <div class="card-body">
                            <h5 class="card-title">Date Nights</h5>
                            <p class="card-text">This lovely salad is quick to make and tastes amazing!</p>
                            <a href="recipes.html" class="btn btn-primary">View date night recipes</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
{/block}